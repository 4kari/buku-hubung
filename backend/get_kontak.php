...
if($_SESSION['level']==1){
    ...
    foreach($wali as $w){
        $id    =$w['id'];
        $w['level'] ='wali';
        $chat       = $conn->query("SELECT * FROM chat WHERE id_guru = '$username' and id_wali = '$id'" , PDO::FETCH_ASSOC)->fetch()['id'];
        if($chat){
            $chat = $chat['id'];
            $w['chat']=$chat;
            $w['pesan'] = count($conn->query("SELECT * FROM pesan WHERE id_chat = '$chat' and terbaca = NULL" , PDO::FETCH_ASSOC)->fetchAll());
        }else{
            $w['pesan'] = 0;
        }
        array_push($data,$w);
...
if($_SESSION['level']==2){
    ...
    foreach($guru as $g){
        ...
        if($chat){
            $chat = $chat['id'];
            $g['chat']=$chat;
            $g['pesan'] = count($conn->query("SELECT * FROM pesan WHERE id_chat = '$chat' and terbaca = NULL" , PDO::FETCH_ASSOC)->fetchAll());
        }else{
            $g['pesan'] = 0;
        }
        array_push($data,$g);
